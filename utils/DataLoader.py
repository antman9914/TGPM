...
def get_link_prediction_data(dataset_name: str, val_ratio: float, test_ratio: float):
    ...
    """
    NODE_FEAT_DIM = EDGE_FEAT_DIM = 768             # Dimension for text-attributed graph.

    if 'tud' not in dataset_name:
        graph_df = pd.read_csv('./processed_data/{}/edge_list.csv'.format(dataset_name))
        ...
        node_interact_times = graph_df.ts.values.astype(np.float32) + 1
        edge_ids = graph_df.idx.values.astype(np.longlong)
        labels = graph_df.label.values

        edge_raw_features = edge_raw_features[labels, :]

    else:
        src_node_ids, dst_node_ids = [], []
        with open('./processed_data/{}/{}_A.txt'.format(dataset_name, dataset_name), 'r') as f:
            lines = f.readlines()
            for line in lines:
                src, dst = line.strip().split(',')
                src_node_ids.append(int(src))
                dst_node_ids.append(int(dst.strip()))
        node_interact_times = []
        with open('./processed_data/{}/{}_edge_attributes.txt'.format(dataset_name, dataset_name), 'r') as f:
            lines = f.readlines()
            for line in lines:
                time = int(line.strip())
                node_interact_times.append(time)
        labels = []
        with open('./processed_data/{}/{}_graph_labels.txt'.format(dataset_name, dataset_name), 'r') as f:
            lines = f.readlines()
            for line in lines:
                label = int(line.strip())
                labels.append(label)
        gids = [1]
        with open('./processed_data/{}/{}_graph_indicator.txt'.format(dataset_name, dataset_name), 'r') as f:
            lines = f.readlines()
            last_gid = 1
            for idx, line in enumerate(lines):
                ...
                if gid != last_gid:
                    ...
                    last_gid = gid
            num_nodes = idx + 1
        src_node_ids, dst_node_ids, node_interact_times, labels, gids = np.array(src_node_ids), np.array(dst_node_ids), np.array(node_interact_times), np.array(labels), np.array(gids)
        edge_raw_features = np.zeros((len(src_node_ids), EDGE_FEAT_DIM))
        node_raw_features = np.zeros((num_nodes, NODE_FEAT_DIM))
    ...
    # padding the features of edges and nodes to the same dimension (768 for all the datasets)
