...
if __name__ == "__main__":
    ...
    datasets = ['enron', 'googlemap', 'icews']
    for data_set_name in datasets:
        print(data_set_name)
        edge_list = pd.read_csv('processed_data/' + data_set_name + '/edge_list.csv')
        num_node = max(edge_list['u'].max(), edge_list['i'].max())
        num_rel = edge_list['r'].max()

        # Prepare datasets
        entity_embeddings = [np.zeros(hidden_size)]
        entity_text_reader = pd.read_csv('processed_data/' + data_set_name + '/entity_text.csv', chunksize=batch_size)
        ...
        np.save('processed_data/' + data_set_name + '/ml_{}_node.npy'.format(data_set_name), entity_embeddings)

        # Prepare datasets
        rel_embeddings = [np.zeros(hidden_size)]
        rel_text_reader = pd.read_csv('processed_data/' + data_set_name + '/relation_text.csv', chunksize=batch_size)
        ...
        np.save('processed_data/' + data_set_name + '/ml_{}.npy'.format(data_set_name), rel_embeddings)